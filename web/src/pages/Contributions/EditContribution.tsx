...
export default function EditContribution() {
  ...
  const fetchContribution = async () => {
    try {
      ...
      setValue('goal', contribution.goal)
      setValue('isActive', contribution.isActive ?? true)
      
      if (contribution.endDate) {
        const date = new Date(contribution.endDate)
      ...
      if (contribution.PaymentMethods && contribution.PaymentMethods.length > 0) {
        ...
        setValue('paymentMethods', formattedMethods)
      }
    } catch (error: any) {
  ...
  const onSubmit = async (data: ContributionForm) => {
    try {
      ...
      toast.success('Campanha de contribuição atualizada com sucesso!')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <h1 className="text-3xl font-bold mb-6">Editar Campanha de Contribuição</h1>
