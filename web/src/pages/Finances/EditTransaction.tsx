...
export default function EditTransaction() {
  ...
  // Resetar formulário quando transactionData mudar
  useEffect(() => {
    if (transactionData && !loading) {
      reset(transactionData)
    }
  }, [transactionData, loading, reset])
  ...
  const fetchTransaction = async () => {
    try {
      ...
      // Preencher campos baseados no tipo
      if (transaction.type === 'ENTRY') {
        formData.entryType = transaction.entryType || undefined
        if (transaction.entryType === 'CONTRIBUICAO') {
          formData.contributionId = transaction.contributionId || undefined
          if (transaction.contributionId) {
            await fetchContributions()
          }
        } else if (transaction.entryType === 'DIZIMO') {
          formData.isTithePayerMember = transaction.isTithePayerMember ?? true
          if (transaction.tithePayerMemberId) {
            formData.tithePayerMemberId = transaction.tithePayerMemberId
            setTithePayerMemberId(transaction.tithePayerMemberId)
          }
          if (transaction.tithePayerName) {
            formData.tithePayerName = transaction.tithePayerName
          }
        }
      } else if (transaction.type === 'EXIT') {
        formData.exitType = transaction.exitType || undefined
        if (transaction.exitType === 'OUTROS') {
          formData.exitTypeOther = transaction.exitTypeOther || undefined
        }
      }
      
      // Armazenar dados para resetar o formulário
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <h1 className="text-3xl font-bold mb-6">Editar Transação</h1>
