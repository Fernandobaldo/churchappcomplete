import { z } from 'zod';
import { generateInviteLink, getAllLinksByBranch, deactivateInviteLink, getInviteLinkByToken, } from '../services/inviteLinkService';
 ...
 */
export async function createInviteLinkController(request, reply) {
    ...
    try {
        ...
        logAudit(request, AuditAction.INVITE_LINK_CREATED, 'MemberInviteLink', `Link de convite criado para filial ${bodyData.branchId}`, {
 ...
 */
export async function getInviteLinksByBranchController(request, reply) {
 ...
 */
export async function deactivateInviteLinkController(request, reply) {
    const paramsSchema = z.object({
        id: z.string().cuid(),
    });
    try {
        ...
        // Log de auditoria
        await logAudit(request, AuditAction.INVITE_LINK_DEACTIVATED, 'MemberInviteLink', `Link de convite desativado: ${id}`, {
            entityId: id,
        });
        // Criar objeto limpo para evitar problemas de serialização
        const responseData = {
 ...
 */
export async function getQRCodeController(request, reply) {
    const paramsSchema = z.object({
        token: z.string(),
    });
    try {
 ...
 */
export async function downloadPDFController(request, reply) {
    const paramsSchema = z.object({
        token: z.string(),
    });
    try {
 ...
 */
export async function getInviteLinkInfoController(request, reply) {
    const paramsSchema = z.object({
        token: z.string(),
    });
    try {
