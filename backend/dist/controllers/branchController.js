...
import { createBranch, getAllBranches, getBranchById, deleteBranchById, } from '../services/branchService';
...
export async function createBranchHandler(request, reply) {
    try {
        ...
        // Log de auditoria
        await AuditLogger.branchCreated(request, branch.id, branch.name, branch.churchId);
        ...
        if (error.message?.includes('permissão') ||
            ...
            if (error.message?.includes('Apenas Administradores') ||
                error.message?.includes('não pode criar')) {
                await AuditLogger.unauthorizedAccessAttempt(request, 'CREATE_BRANCH', error.message);
            }
            // Log de limite excedido
            if (error.message?.includes('Limite do plano')) {
                const limitMatch = error.message.match(/(\d+)\s*filiais/);
                if (limitMatch) {
                    await AuditLogger.planLimitExceeded(request, 'branches', parseInt(limitMatch[1]), parseInt(limitMatch[1]));
                }
            }
