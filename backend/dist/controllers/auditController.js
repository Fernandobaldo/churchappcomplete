import { z } from 'zod';
import { getAuditLogs, getMemberAuditLogs, getBranchAuditLogs, getUserAuditLogs, } from '../services/auditService';
...
export async function getMemberAuditLogsHandler(request, reply) {
    try {
        const user = request.user;
        if (!user || !user.memberId) {
            return reply.status(401).send({ error: 'Autenticação necessária' });
...
export async function getBranchAuditLogsHandler(request, reply) {
    try {
        const user = request.user;
        if (!user || !user.memberId) {
            return reply.status(401).send({ error: 'Autenticação necessária' });
