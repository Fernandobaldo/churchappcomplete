...
export async function generateInviteLink(data) {
    ...
    try {
        ...
        if (hasLimiteAndPlano || isPlanLimitReachedMessage || isPlanLimitReachedCode) {
            const limitError = new Error('Limite de membros do plano atingido. Não é possível criar novos links de convite.');
            ...
            if (plan.maxMembers !== null) {
                ...
                if (totalMembers >= plan.maxMembers) {
                    ...
                    if (adminEmails.length > 0) {
                        try {
                            await sendMemberLimitReachedNotification(adminEmails, inviteLink.Branch.Church.name, totalMembers, plan.maxMembers);
                        }
                        catch (emailError) {
 ...
 */
export async function incrementLinkUsage(token) {
    const inviteLink = await prisma.memberInviteLink.findUnique({
        where: { token },
    });
    if (!inviteLink) {
 ...
 */
export async function deactivateInviteLink(linkId, userId) {
    // 1. Buscar o link
    const inviteLink = await prisma.memberInviteLink.findUnique({
        where: { id: linkId },
        include: {
    ...
    if (inviteLink.createdBy !== userId) {
        ...
        if (memberWithPermissions.role !== 'ADMINGERAL' &&
 ...
 */
export async function getActiveLinksByBranch(branchId, userId) {
    // Validar permissões
    const creatorMember = await getMemberFromUserId(userId);
    if (!creatorMember) {
        throw new Error('Membro não encontrado');
 ...
 */
export async function getAllLinksByBranch(branchId, userId) {
    // Validar permissões
    const creatorMember = await getMemberFromUserId(userId);
    if (!creatorMember) {
        throw new Error('Membro não encontrado');
