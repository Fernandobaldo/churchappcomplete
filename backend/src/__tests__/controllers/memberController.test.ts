...
describe('updateMemberById', () => {
  ...
  describe('Validações de updateMemberById', () => {
    ...
    it('deve validar permissão para alterar cargo', async () => {
      ...
      expect(validatePositionChangePermission).toHaveBeenCalledWith(
        VALID_CUID_MEMBER,
        VALID_CUID_MEMBER,
        Role.MEMBER,
        []
    ...
    it('deve rejeitar alteração de cargo sem permissão', async () => {
      vi.mocked(validatePositionChangePermission).mockRejectedValue(
        new Error('Você não tem permissão para alterar o cargo')
      )
    ...
    it('deve retornar dados atualizados corretamente', async () => {
      ...
      vi.mocked(updateMember).mockResolvedValue(updatedMember as any)
      ...
      const responseData = vi.mocked(mockReply.send).mock.calls[0][0] as any
    ...
    it('deve fazer log de auditoria após atualização', async () => {
      ...
      expect(AuditLogger.memberUpdated).toHaveBeenCalledWith(
        request,
        VALID_CUID_MEMBER,
        expect.objectContaining({
          name: 'Nome Atualizado',
