...
export async function registerController(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    ...
    const serviceData: any = { ...bodyData }
    ...
    if (isInviteRegistration) {
      ...
      try {
        ...
        await logAudit(
          ...
          'Member',
          `Membro registrado via link de convite: ${result.email}`,
          {
            entityId: result.id,
            metadata: {
        ...
        if (error.message === 'LIMIT_REACHED') {
          ...
          try {
            await logAudit(
              request,
              AuditAction.MEMBER_REGISTRATION_ATTEMPT,
              'Member',
              `Tentativa de registro via link de convite bloqueada: limite de membros atingido`,
              {
                metadata: {
