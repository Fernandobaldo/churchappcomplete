...
import {
  ...
  serviceScheduleParamsSchema,
  branchIdParamsSchema,
  createEventsSchema,
  deleteServiceScheduleSchema,
} from '../schemas/serviceScheduleSchemas'
import { getMemberFromUserId } from '../utils/authorization'
import { logAudit } from '../utils/auditHelper'
import { AuditAction } from '@prisma/client'

export class ServiceScheduleController {
  private service = new ServiceScheduleService()
  ...
  async create(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      await logAudit(
        request,
        AuditAction.SERVICE_SCHEDULE_CREATED,
        'ServiceSchedule',
        `Horário de culto '${schedule.title}' criado para a filial ${user.branchId}`,
        {
      ...
      if (schedule.autoCreateEvents) {
        try {
          const eventsResult = await this.service.createEventsFromSchedule(
            schedule.id,
  ...
  async getByBranch(request: FastifyRequest, reply: FastifyReply) {
    try {
    ...
    try {
      ...
      try {
        ...
        const hasRelevantChanges =
          (data.title !== undefined && data.title !== oldSchedule.title) ||
        ...
        if (hasRelevantChanges) {
          ...
          if (updatedEventsCount > 0) {
            await logAudit(
              ...
              'ServiceSchedule',
              `${updatedEventsCount} evento(s) atualizado(s) junto com o horário '${updatedSchedule.title}'`,
              {
      ...
      await logAudit(
        ...
        AuditAction.SERVICE_SCHEDULE_UPDATED,
        'ServiceSchedule',
        `Horário de culto '${updatedSchedule.title}' atualizado para a filial ${user.branchId}`,
        {
          entityId: updatedSchedule.id,
          metadata: {
            scheduleTitle: updatedSchedule.title,
  ...
  async getRelatedEventsCount(request: FastifyRequest, reply: FastifyReply) {
    try {
      const { id } = serviceScheduleParamsSchema.params.parse(request.params)
  ...
  async delete(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      if (deleteEvents && eventsCount > 0) {
        ...
        await logAudit(
          request,
          AuditAction.EVENTS_DELETED_FROM_SCHEDULE,
          'ServiceSchedule',
          `${deletedEventsCount} eventos deletados junto com o horário '${schedule.title}'`,
          {
            entityId: id,
            metadata: {
              scheduleTitle: schedule.title,
      ...
      await logAudit(
        request,
        AuditAction.SERVICE_SCHEDULE_DELETED,
        'ServiceSchedule',
        `Horário de culto '${schedule.title}' deletado da filial ${user.branchId}`,
        {
          entityId: id,
          metadata: {
            scheduleTitle: schedule.title,
  ...
  async createEvents(request: FastifyRequest, reply: FastifyReply) {
    try {
      const { id } = serviceScheduleParamsSchema.params.parse(request.params)
      ...
      const result = await this.service.createEventsFromSchedule(
        id,
        body.startDate,
        body.endDate,
        body.daysAhead
